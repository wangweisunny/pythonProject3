rule Malware_Generic
{
    ...
    strings:
        ...
        $api_call = { 6A 00 E8 00 00 00 00 83 C4 04 } // Common API call pattern (with fixed bytes)
        $packed_code = { 55 8B EC 83 EC 10 33 C0 60 } // Packed code pattern
        $shellcode = { E8 00 00 00 00 83 C4 04 } // Shellcode jump pattern

    condition:
        uint16(0) == 0x5A4D and // Check for MZ header
        filesize > 1024 and // Ensure file is larger than 1KB
        1 of ($xor_pattern, $api_call, $packed_code, $shellcode) // Match at least 1 of the patterns
}